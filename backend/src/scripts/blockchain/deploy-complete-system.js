...
async function deployCompleteSystem() {
    ...
    try {
        ...
        try {
            ...
            throw new Error(`❌ Format de clé privée invalide: ${error.message}`);
        }
        ...
        try {
            ...
            const accountBalance = await accountId.getBalance(client);
            console.log(`Solde actuel: ${accountBalance.toString()}`);

            if (accountBalance.toBigNumber().lt(10000000000)) { // Moins de 10 HBAR (en tinybars)
            ...
            console.warn("⚠️ Impossible de vérifier le solde:", error.message);
        }
